...
from PyQt5.QtWidgets import (QDialog, QLabel, QLineEdit, QSpinBox, QDoubleSpinBox, QCheckBox,
                            QDialogButtonBox, QTabWidget, QWidget, QHBoxLayout, QGridLayout,
                            QVBoxLayout, QApplication, QDateEdit) 
...
class PaymentDlg(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        for lineEdit in (self.forenameLineEdit, self.surnameLineEdit, self.checkNumLineEdit,
                        self.accountNumLineEdit, self.bankLineEdit, self.sortCodeLineEdit,
                        self.creditCardLineEdit):
            lineEdit.textEdited['QString'].connect(self.updateUi)

        for dateEdit in (self.validFromDateEdit, self.expiryDateEdit):
            dateEdit.dateChanged['QDate'].connect(self.updateUi)
