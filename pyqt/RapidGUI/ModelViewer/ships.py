...
import re
import operator
from PyQt5.QtCore import (Qt, pyqtSignal, QFile, QIODevice, QDataStream, QAbstractTableModel,
    QModelIndex, QSize, QTextStream, QFileInfo)
from PyQt5.QtGui import QColor, QTextDocument
from PyQt5.QtWidgets import (QStyledItemDelegate, QApplication,
    QStyle, QComboBox, QLineEdit, QTextEdit, QLineEdit, QFileDialog)
...
class ShipContainer:
    ...
    def __iter__(self):
        ...
        #     yield ship
        yield from (ship for ship in self.ships.values())
    ...
    def inCountryOwnerOrder(self):
        return sorted(self.ships.values(),
                key=operator.attrgetter("country", "owner", "name"))
        # return sorted(self.ships.values(),
...
class ShipTableModel(QAbstractTableModel):
    ...
    def sortByName(self):
        # Use beginResetModel and endResetModel instead of the obsolete
        # reset() method.
    ...
    def sortByTEU(self):
        self.beginResetModel()
        self.ships = sorted(self.ships,
                    key=operator.attrgetter('teu', 'country', 'owner', 'name'))
        self.endResetModel()
    ...
    def data(self, index, role=Qt.DisplayRole):
        ...
        if role == Qt.DisplayRole:
            ...
            if column == NAME:
                return "{} (min. 3 characters)".format(ship.name)
            elif column == OWNER:
                return "{} (min. 3 characters)".format(ship.owner)
            elif column == COUNTRY:
                return "{} (min. 3 characters)".format(ship.country)
            elif column == DESCRIPTION:
                return ship.description
            elif column == TEU:
    ...
    def exportToTxt(self, fname):
        ...
        try:
            ...
            if error is not None:
                return False, error
            return True, "Saved {} ship records to {}".format(len(self.ships),
...
class ShipDelegate(QStyledItemDelegate):
    ...
    def setModelData(self, editor, model, index):
        ...
        elif index.column() in (OWNER, COUNTRY) and len(editor.currentText) >= 3:
