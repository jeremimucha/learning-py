...
class Movie(object):
    ...
    def __init__(self, title=None, year=UNKNOWNYEAR,
                 minutes=UNKNOWNMINUTES, acquired=None, notes=None, location=UNKNOWNLOCATION):
        self.title = title
        self.year = year
...
class MovieContainer(object):
    ...
    def __iter__(self):
        yield from (pair[1] for pair in self.__movies)
    ...
    def loadQDataStream(self):
        ...
        try:
            ...
            if version != MovieContainer.FILE_VERSION and \
               version != MovieContainer.OLD_FILE_VERSION:
                raise IOError("Unreadable file version: {}".format(version))
            stream.setVersion(QDataStream.Qt_5_7)
            self.clear(False)
    ...
    def savePickle(self):
        ...
        try:
            ...
            pickle.dump(self.__movies, fh, pickle.HIGHEST_PROTOCOL)
        except EnvironmentError as e:
            error = "Failed to save: {}".format(e)
        finally:
            if fh is not None:
        ...
        try:
            ...
            for key, movie in self.__movies:
                stream << ("<MOVIE YEAR='{}' MINUTES='{}' "
                           "ACQUIRED='{}' LOCATION='{}'>\n".format(movie.year,
                           movie.minutes, movie.acquired.toString(Qt.ISODate), movie.location)) \
    ...
    def endElement(self, namespaceURI, localName, qName):
        if qName == "MOVIE":
            if (self.year is None or self.minutes is None or
                ...
                self.notes is None or self.location is None):
